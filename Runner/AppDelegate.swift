...
//
//            let poseClassifierProcessorData = results?.map { pose in
//                return poseClassifierProcessor.getPoseResult(pose: pose)
//            } ?? []
    ...
    func poseToDictionary(pose: Pose) -> [String: Any] {
        var classifier = self.poseClassifierProcessor.getPoseResult(pose: pose)
        ...
        poseDictionary["classifier"] = classifier
